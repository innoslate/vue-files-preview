...
const Z = /* @__PURE__ */ new WeakMap(), oe = /* @__PURE__ */ new WeakMap();
var S;
(function(l) {
...
class z {
  ...
  */
  resolveInner(e, t = 0) {
    let r = K(oe.get(this) || this.topNode, e, t, !0);
...
class H {
  ...
  */
  findChild(e, t, r, n, i) {
    ...
    for (let h = e; h != t && !(be(i, n, s[h + 1], s[h + 2]) && (f = h, r > 0)); h = s[h + 3])
      ;
    return f;
  }
  /**
...
class B extends we {
  ...
  nextChild(e, t, r, n, i = 0) {
    for (let s = this; ; ) {
      for (let { children: f, positions: h } = s._tree, u = t > 0 ? f.length : -1; e != u; e += t) {
        let o = f[e], c = h[e] + s.from;
        if (be(n, r, c, c + o.length)) {
          if (o instanceof H) {
            if (i & S.ExcludeBuffers)
              continue;
            let d = o.findChild(0, o.buffer.length, t, r - c, n);
            if (d > -1)
              return new D(new Ae(s, o, e, c), null, d);
          } else if (i & S.IncludeAnonymous || !o.type.isAnonymous || se(o)) {
            let d;
            if (!(i & S.IgnoreMounts) && (d = q.get(o)) && !d.overlay)
              return new B(d.tree, c, e, s);
            let x = new B(o, c, e, s);
            return i & S.IncludeAnonymous || !x.type.isAnonymous ? x : x.nextChild(t < 0 ? o.children.length - 1 : 0, t, r, n);
          }
        }
      }
...
function Ie(l) {
  var e;
  let { buffer: t, nodeSet: r, maxBufferLength: n = 1024, reused: i = [], minRepeatType: s = r.types.length } = l, f = Array.isArray(t) ? new ie(t, t.length) : t, h = r.types, u = 0, o = 0;
  function c(m, w, a, C, y, A) {
    let { id: g, start: p, end: b, size: v } = f, N = o, U = u;
    for (; v < 0; )
      if (f.next(), v == -1) {
        let W = i[g];
        a.push(W), C.push(p - m);
    ...
    if (b - p <= n && (J = O(f.pos - w, y))) {
      ...
      if (V >= 0 && G > 0 && G < M.length && _(M, R, p, G, p, Y, V, N, U), M.reverse(), R.reverse(), V > -1 && G > 0) {
        let he = x($, U);
        X = le($, M, R, 0, M.length, 0, b - p, he, he);
      } else
        X = P($, M, R, b - p, N - b, U);
    }
    a.push(X), C.push(fe);
  ...
  function d(m, w, a, C) {
    ...
    for (; f.pos > w; ) {
      ...
      if (N > 4)
        ...
        if (g > -1 && b < g)
          break;
        g < 0 && (g = v - n), y.push(p, b, v), A++, f.next();
      }
    }
    if (A) {
      let p = new Uint16Array(A * 4), b = y[y.length - 2];
      for (let v = y.length - 3, N = 0; v >= 0; v -= 3)
        p[N++] = y[v], p[N++] = y[v + 1] - b, p[N++] = y[v + 2] - b, p[N++] = N;
      a.push(new H(p, y[2] - b, r)), C.push(b - m);
    }
  }
  function x(m, w) {
    return (a, C, y) => {
      ...
      if (g >= 0 && (p = a[g]) instanceof z) {
        if (!g && p.type == m && p.length == y)
          return p;
        (b = p.prop(k.lookAhead)) && (A = C[g] + p.length + b);
      }
      return P(m, a, C, y, A, w);
    };
  }
  function _(m, w, a, C, y, A, g, p, b) {
    let v = [], N = [];
    for (; m.length > C; )
      v.push(m.pop()), N.push(w.pop() + a - y);
    m.push(P(r.types[g], v, N, A - y, p - A, b)), w.push(y - a);
  }
  function P(m, w, a, C, y, A, g) {
    if (A) {
  ...
  function O(m, w) {
    let a = f.fork(), C = 0, y = 0, A = 0, g = a.end - n, p = { size: 0, start: 0, skip: 0 };
    e: for (let b = a.pos - m; a.pos > b; ) {
      let v = a.size;
      if (a.id == w && v >= 0) {
        p.size = C, p.start = y, p.skip = A, A += 4, C += 4, a.next();
        continue;
      }
      let N = a.pos - v;
    ...
    return (w < 0 || C == m) && (p.size = C, p.start = y, p.skip = A), p.size > 4 ? p : void 0;
  }
  function F(m, w, a) {
    let { id: C, start: y, end: A, size: g } = f;
    if (f.next(), g >= 0 && C < s) {
      let p = a;
      if (g > 4) {
        let b = f.pos - (g - 4);
      ...
      w[--a] = p, w[--a] = A - m, w[--a] = y - m, w[--a] = C;
    } else g == -3 ? u = C : g == -4 && (o = C);
    return a;
  }
  let E = [], I = [];
  for (; f.pos > 0; )
  ...
  function x(_, P, O, F, E) {
    for (let I = O; I < F; ) {
      ...
      if (I == L + 1) {
        if (w > o) {
          let a = _[L];
          x(a.children, a.positions, 0, a.children.length, P[L] + E);
          continue;
        }
        c.push(_[L]);
      } else {
        let a = P[I - 1] + _[I - 1].length - m;
        c.push(le(l, _, P, L, I, m, a, null, h));
      }
      d.push(m + E - i);
    }
  }
  return x(e, t, r, n, 0), (f || h)(c, d, s);
}
class Fe {
  constructor() {
      ...
      if (c - h >= r)
        for (; s && s.from < c; ) {
          ...
          if (h >= d.from || c <= d.to || u) {
            let x = Math.max(d.from, h) - u, _ = Math.min(d.to, c) - u;
            d = x >= _ ? null : new Q(x, _, d.tree, d.offset + u, f > 0, !!o);
          }
  ...
  startInner() {
    ...
    e: for (let i, s; ; ) {
      ...
      if (this.stoppedAt != null && n.from >= this.stoppedAt)
        ...
        if (t) {
          let u = t.mounts.find((o) => o.frag.from <= n.from && o.frag.to >= n.to && o.mount.overlay);
          if (u)
            for (let o of u.mount.overlay) {
              let c = o.from + u.pos, d = o.to + u.pos;
              c >= n.from && d <= n.to && !t.ranges.some((x) => x.from < d && x.to > c) && t.ranges.push({ from: c, to: d });
            }
        }
        f = !1;
        ...
        if (typeof i.overlay == "function")
          ...
          let o = me(this.ranges, i.overlay || (n.from < n.to ? [new T(n.from, n.to)] : []));
          o.length && de(o), (o.length || !i.overlay) && this.inner.push(new pe(i.parser, o.length ? i.parser.startParse(this.input, xe(u, o), o) : i.parser.startParse(""), i.overlay ? i.overlay.map((c) => new T(c.from - n.from, c.to - n.from)) : null, n.tree, o.length ? o[0].from : n.from)), i.overlay ? o.length && (r = { ranges: o, depth: 0, prev: r }) : f = !1;
        }
      } else if (t && (h = t.predicate(n)) && (h === !0 && (h = new T(n.from, n.to)), h.from < h.to)) {
      ...
      if (f && n.firstChild())
        ...
        for (; !n.nextSibling(); ) {
          ...
          if (t && !--t.depth) {
            ...
            u.length && (de(u), this.inner.splice(t.index, 0, new pe(t.parser, t.parser.startParse(this.input, xe(t.mounts, u), u), t.ranges.map((o) => new T(o.from - t.start, o.to - t.start)), t.target, u[0].from))), t = t.prev;
          }
          r && !--r.depth && (r = r.prev);
        }
    }
...
function Ee(l) {
  ...
  function u(o, c, d, x, _, P) {
    let O = t[P], F = [], E = [];
    ce(s, o, O, F, E, x);
    let I = f[O + 1], L = f[O + 2];
    h.push(F.length);
    let m = P ? u(O + 4, f[O + 3], s.set.types[f[O]], I, L - I, P - 1) : e.toTree();
    return F.push(m), E.push(I - x), ce(s, f[O + 3], c, F, E, x), new z(d, F, E, _);
  }
  n.children[i] = u(0, f.length, j.none, 0, s.length, t.length - 1);
  for (let o of h) {
    let c = l.tree.children[o], d = l.tree.positions[o];
    l.yield(new B(c, d + l.from, o, l._tree));
  }
}
class ge {
  constructor(e, t) {
...
function Te(l, e, t, r) {
  let n = 0, i = 0, s = !1, f = !1, h = -1e9, u = [];
  for (; ; ) {
    let o = n == l.length ? 1e9 : s ? l[n].to : l[n].from, c = i == e.length ? 1e9 : f ? e[i].to : e[i].from;
    if (s != f) {
      let d = Math.max(h, t), x = Math.min(o, c, r);
      d < x && u.push(new T(d, x));
    }
    if (h = Math.min(o, c), h == 1e9)
      break;
    o == h && (s ? (s = !1, n++) : s = !0), c == h && (f ? (f = !1, i++) : f = !0);
  }
  return u;
}
function xe(l, e) {
  ...
  for (let { pos: r, mount: n, frag: i } of l) {
    let s = r + (n.overlay ? n.overlay[0].from : 0), f = s + n.tree.length, h = Math.max(i.from, s), u = Math.min(i.to, f);
    if (n.overlay) {
      let o = n.overlay.map((d) => new T(d.from + r, d.to + r)), c = Te(e, o, h, u);
      for (let d = 0, x = h; ; d++) {
        let _ = d == c.length, P = _ ? u : c[d].from;
        if (P > x && t.push(new Q(x, P, n.tree, -s, i.from >= x || i.openStart, i.to <= P || i.openEnd)), _)
          break;
        x = c[d].to;
      }
    } else
      t.push(new Q(h, u, n.tree, -s, i.from >= s || i.openStart, i.to <= f || i.openEnd));
  }
  return t;
}
export {
